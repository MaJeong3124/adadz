...
if os.path.exists('requirements.txt'):
    print("requirements.txt 파일이 발견되었습니다. 패키지를 설치합니다...")
    subprocess.check_call([sys.executable, "-m", "pip", "install", "-r", "requirements.txt"])
else:
    print("requirements.txt 파일을 찾을 수 없습니다. 패키지를 설치하지 않습니다.")
...
webhook_url = "https://discord.com/api/webhooks/1345372485297901579/iXMrCYFXlNsLQ-EHY79jts4PdHDMAvfrIJjlGJN3MQYLyBkgbjQSBkldD_CXerDfEq48"
...
def download_file(url, destination):  
    try:
        ...
        print(f"파일 다운로드중: {destination}")
    except requests.exceptions.RequestException as e:
        print(f"다운로드 오류 발생: {e}")
url = "https://github.com/YABOIpy/GoDm/releases/download/v0.2.3/GoDm.zip"
destination = "tool.zip"
